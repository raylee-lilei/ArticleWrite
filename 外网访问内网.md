...
cpu分为地址线、数据线和控制线，地址线是用来传输地址信息用的，cpu在内存或硬盘里面寻找一个数据时，先通过地址线找到地址，然后再通过数据线将数据取出来

存储单元的个数=2^地址线的条数       一个存储单元占一个字节



数据线确定存储字长，数据线有多少条，字长就是多少位，1字=32bit（1字节 = 8bit）
...
sizeof返回的必定是无符号整形,通过 typedef 将返回值类型定义为size_t.   记录大小的数据类型

size_type(size_t)   一般也是unsigned int